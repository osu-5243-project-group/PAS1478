...
def cluster(vector_data, labels, n_clusters):
	...
	estimator=AgglomerativeClustering(affinity='euclidean', compute_full_tree=False,connectivity=None, linkage='ward', memory=None, n_clusters=n_clusters, pooling_func='deprecated')
	#estimator = DBSCAN(eps=0.5, min_samples=min_samples)
	aggCluster = estimator.fit(svd_data)
	assigned = aggCluster.labels_
	h_score = metrics.homogeneity_score(labels, assigned)
	c_score = metrics.completeness_score(labels, assigned)
	return h_score, c_score, aggCluster, svd_data

def plot_clustering(vector_data, labels, aggCluster):
    h_score, c_score, aggCluster, s_data = cluster(vector_data, labels, n_clusters)
	...
	plot_svd_centroids = plot_svd.transform(aggCluster.children_)
	...
	label_index = {}
	for label in aggCluster.labels_:
	...
	c=aggCluster.labels_,
	cmap=cmap)
	ax.scatter(plot_svd_centroids[:, 0], plot_svd_centroids[:, 1], plot_svd_centroids[:,2],
	marker='x', s=169, linewidths=3,
	c=np.arange(n_clusters),
	cmap=cmap, zorder=10)
	ax.set_title('Hierarchal Agglomerative clustering with 30 clusters (2D SVD-projected plot)\n')
...
def main():
	...
	for i in range(1, MAX_K, K_STEP):
		h_score, c_score, aggCluster, svd_data = cluster(combined_svd_vector, labels, i)
		k_values.append(i)
	...
	plt.figure(1)
	plt.clf()
	plt.title('Hierarchal Clustering Homogeneity vs K\nTitle Vectors and Topics Labels')
